...
string dcYHuffmanTable[12] = {"00", "010", "011", "100", "101", "110", "1110", "11110", "111110", "1111110", "11111110", "111111110"};
string dcCbCrHuffmanTable[12] = {"00", "01", "10", "110", "1110", "11110", "111110", "1111110", "11111110", "111111110", "1111111110", "11111111110"};
...
void decode(bool isRGB)
{
    ...
    */
    ...
    while (1)
    {
        ...
        {
            if (blockNum > ((width / 8) * (height / 8) + (width / 16) * (height / 16) * 2 - 1)) // prevent decode too much block
            {
                break;
            }
            ...
            else
            {
                if (remainBit < 0)
                {
                    ...
                    else if ((acCbCrDecodeTable.find(decode) == acCbCrDecodeTable.end() && phase != 0)) // not found in cb cr ac table
                    {
                        continue;
                    }
                    else
                    {
                        // if receive EOB
                        if ((acYDecodeTable.find(decode) != acYDecodeTable.end() && acYDecodeTable.find(decode)->second == "0/0" && phase == 0) || (acCbCrDecodeTable.find(decode) != acCbCrDecodeTable.end() && acCbCrDecodeTable.find(decode)->second == "0/0" && phase != 0))
                        {
                            ...
                            for (int i = 0; i < 8; i++)
                            {
                                for (int j = 0; j < 8; j++)
                                {
                                    if (phase == 0)
                                    {
                                        decodeimageData[(((blockNum % ((height / 8) * (width / 8))) / (width / 8)) * 8) + i][(((blockNum % ((height / 8) * (width / 8))) % (width / 8)) * 8) + j][phase] = result[i][j] + 128;
                                    }
                                    else
                                    {
                                        int start;
                                        ...
                                        if (phase == 2)
                                        {
                                            start = (blockNum % ((height / 8) * (width / 8) + ((height / 2 / 8) * (width / 2 / 8))));
                                        }
                                        decodeimageData[((start / (width / 8 / 2)) * 16) + i * 2][((start % (width / 8 / 2)) * 16) + j * 2][phase] = result[i][j];
                                        decodeimageData[((start / (width / 8 / 2)) * 16) + i * 2 + 1][((start % (width / 8 / 2)) * 16) + j * 2][phase] = result[i][j];     // down
                                        decodeimageData[((start / (width / 8 / 2)) * 16) + i * 2][((start % (width / 8 / 2)) * 16) + j * 2 + 1][phase] = result[i][j];     // right
                                        decodeimageData[((start / (width / 8 / 2)) * 16) + i * 2 + 1][((start % (width / 8 / 2)) * 16) + j * 2 + 1][phase] = result[i][j]; // right down
                                    }
                                }
                            }
                            ...
                            if (blockNum == ((height / 8) * (width / 8) + ((height / 2 / 8) * (width / 2 / 8)))) // Cb is done
                            {
                                isDc = true;
                                prevDC = 0;
                                phase = 2;
...
void test() // just for quick testing
{
    ...
    string filename[4] = {"GrayImages/Baboon.raw", "GrayImages/Lena.raw", "ColorImages/BaboonRGB.raw", "ColorImages/LenaRGB.raw"};
    for (int i = 0; i < 4; i++)
    {
